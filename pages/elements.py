...
class WebElement(object):
    ...
    def find(self, timeout=1):
        ...
        try:
            element = WebDriverWait(self._web_driver, timeout).until(
               es.presence_of_element_located(self._locator)
            )
        except Exception:
    ...
    def wait_to_be_clickable(self, timeout=5, check_visibility=True):
        ...
        try:
            element = WebDriverWait(self._web_driver, timeout).until(
                es.element_to_be_clickable(self._locator)
            )
        except Exception:
    ...
    def wait_until_not_visible(self, timeout=5):
        ...
        try:
            element = WebDriverWait(self._web_driver, timeout).until(
                es.visibility_of_element_located(self._locator)
            )
        except Exception:
    ...
    def find(self, timeout=1):
        ...
        try:
            elements = WebDriverWait(self._web_driver, timeout).until(
               es.presence_of_all_elements_located(self._locator)
            )
        except Exception:
